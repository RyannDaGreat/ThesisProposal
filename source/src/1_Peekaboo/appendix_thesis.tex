...
\caption[Varying Segmentation Granularity]{\textbf{Varying granularity}:
...
\end{minipage}
\caption[Open Vocabulary Localization Examples]{\textbf{\modelname is truly open vocabulary}:
...
\subsubsection{Alpha Regularization}
    ...
    \begin{figure}[!h]
        ...
        \caption[Alpha Regularization Effect on Segmentation]{The alpha regularization term plays a large role when using Peekaboo to segment images. If $\gravco$ is too large (on the far right), the entire image might be ignored. Conversely, if $\gravco$ is too small, tons of background details are included.
     }
...
\begin{minipage}{0.49\linewidth}
    \caption[Ablation on Implicit Neural Representation]{\textbf{Ablation on implicit neural representation}:
...
\begin{minipage}{0.72\linewidth}
    \caption[Visualizing Implicit Neural Representation]{\textbf{Visualizing implicit neural representation}:
...
% \vspace{1em}
\begin{minipage}{\linewidth}
    \caption[Ablation on Bilateral Filters]{\textbf{Ablation on bilateral filters}:
...
\end{minipage}
\caption[Limitations and Failure Cases]{\textbf{Limitations and failures of \modelname }:
...
\caption[Stable Diffusion Generated Images]{\textbf{Images generated from Stable Diffusion:} 
...
\end{minipage}
\begin{minipage}{0.96\textwidth}
\caption[Foreground-Background Separation Experiment]{\textbf{Can diffusion models separate foreground and background?}
...
\caption[Extended Analogous Experiment Visualization]{\textbf{Extended visualization of analogous experiment}
}
...
\begin{figure}[t]
	...
	\includegraphics[width=0.95\linewidth]{src/1_Peekaboo/figures/Pascal-per-class-iou.png}
		\caption[Per-Class IoU Values]{\textbf{Per-Class IoU values:} We report results on RefCOCO-C (left) and Pascal VOC-C (right) on a per-class basis. For RefCOCO-C, text captions were used to segment, and results were categorized according to the class of the referred object.}
...
\caption[Eye Segmentation Examples]{In this figure, we play a game: find the eyes! Peekaboo can segment specific features of images, and is good at finding eyes. Only one prompt was used: \texttt{eyes}. On the top row we have input images, and in the middle row we have the outputted alpha map. On the bottom we overlay the input images with a white mask corresponding to the alpha, to better show which part of the image it chose to segment.
}
...
\caption[RefCOCO-C Segmentation Results]{Our results on RefCOCO-C. For each sample we demonstrate the prompt text(title), input image(left), our output alpha mask(middle), and the image segmented by the mask(right)
}
...
\caption[Pascal VOC-C Segmentation Results]{Our results on Pascal VOC-C. For each sample we demonstrate the prompt text(title), input image(left), our output alpha mask(middle), and the image segmented by the mask(right)
}
...
\begin{figure}[!h]
    ...
    \caption[Segmentation on Diverse Images]{Peekaboo's segmentation results on various images, including pop references such as Avatar the Last Airbender and AI-generated images of imaginary objects that don't exist such as avocado armchairs. Prompt and input image are on the left, and the alpha mask output is on the right.
    }
    \label{peekaboo_fig:coco_res}
    % 
\end{figure}
...
% \subsubsection{Transparent Image Generation}
    \begin{figure}[!h]
        ...
        \caption[RGBA Generation Timelapses Part 1]{\textbf{Part 1/2.} Peekaboo can generate images with transparency masks! Continuing from \cref{peekaboo_fig:rgbaexamples}, we display timelapses of the transparent image generation task described in \cref{peekaboo_sec:rgbagen}. The prompt for each image is to its right.
     }
    ...
    \begin{figure}[!h]
        ...
        \caption[RGBA Generation Timelapses Part 2]{\textbf{Part 2/2.} Peekaboo can generate images with transparency masks! Continuing from \cref{peekaboo_fig:rgbaexamples}, we display timelapses of the transparent image generation task described in \cref{peekaboo_sec:rgbagen}. The prompt for each image is to its right.
         }
        \label{peekaboo_fig:extrargbapart1}
        % 
    \end{figure}
